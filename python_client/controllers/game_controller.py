...
import LoginModule # For LoginModule.AlreadyLoggedInException
...
class GameController:
    ...
    def _launch_login_app(self):
        """Launches the PyQt5 login application in a new process."""
        try:
            # Correctly locate the login app (app.py)
            base_dir = os.path.dirname(os.path.abspath(__file__))
            login_app_path = os.path.join(base_dir, '..', 'app.py')
            python_executable = sys.executable
            command = [python_executable, login_app_path]
            
            print(f"Relaunching login app with command: {' '.join(command)}")
            subprocess.Popen(command)
            
            # Close the main game window
            self.app_view.destroy()
        except Exception as e:
            print(f"Error launching login app: {e}")
    ...
    def _poll_mp_lobby_state(self):
        ...
        if not self.polling_active and not match_found:
    ...
    def _poll_mp_game_state(self):
        ...
        while self.polling_active and self.current_view == mp_game_view:
            try:
                ...
                # State for the scores panel to prevent flickering
                current_scores_panel_state = (
                    frozenset(players),
                    frozenset(scores.items()),
                    pov_username,
                    my_player_is_done_this_round or not round_in_progress_server
                )
                ...
                self.app_view.after(0, lambda pov_u=pov_username, p_guesses=dict(player_guesses_map), act_words=dict(all_current_words), c_truly_g=can_truly_guess, interact_o=interaction_over_for_pov, wd=word_display, td=timer_display, rd=round_display, sd=status_display, pl=list(players), sc=dict(scores), iudgfsb=is_user_done_guessing_for_spectate_button, sh_upd_kb=should_update_keyboard, sh_upd_sc=should_update_scores: 
                    mp_game_view.update_display(wd, td, rd, sd, pl, sc, pov_u, p_guesses, act_words, c_truly_g, iudgfsb, interact_o, sh_upd_kb, sh_upd_sc)
                )
